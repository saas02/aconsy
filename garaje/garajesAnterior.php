...
<body>
    ...
    <p>
		...
		error_reporting(0);
			...
			if($_SESSION['id_garaje']==1){
			$_SESSION['id_garaje']=($maxid_garaje)+1;
				}else{
			$_SESSION['id_garaje']=$id_garaje;
				}
				if($id_garaje==""){
			$Sql="select id_garaje, lugar_garaje, numero_garaje from garaje where id_garaje='".$maxid_garaje."'";
			$reultid_vehi=query($Sql,$link); 
			$datosid_vehi=mysql_fetch_array($reultid_vehi);
			$id_garaje=$datosid_vehi[0];
 			$lugar_garaje=$datosid_vehi[1];
...
<form name="asigGaraje" action="garajesSiguiente.php" method="post">
	...
	<table align="center" border="0">
    	<tr>
        	...
        	<td>
            	<input class="btn btn-danger" type="button" value="Anterior" onclick="window.location='garajesAnterior.php'"  />
            </td>
            <td>
        ...
        <tr>
            ...
            <td align="center" colspan="2">
            	<input class="btn btn-danger" type="button" value="Registrar Nuevo Garaje" onclick="window.location='garajesCrear.php'" />
            </td>
        </tr>
    </table>
        </div>
