...
$Sql="select v.id_vehiculo, placa, marca, color, tipo from vehiculo as v inner join usuario_vehiculo as uv on v.id_vehiculo=uv.id_vehiculo where ".$id_usu." = uv.id_usuario";
			...
			$id_ingreso=$_SESSION['id_ingreso'];
			$fecha_salida=$_SESSION['fecha'];
			$Sql="UPDATE ingreso SET fecha_salida = '".$fecha_salida."' WHERE id_ingreso = '".$id_ingreso."'";
			query($Sql,$link);
			$Sql="select fecha_entrada from ingreso_vehiculo as iv inner join ingreso as i on i.id_ingreso=iv.id_ingreso where id_usuario = ".$id_usu." and iv.id_ingreso=".$id_ingreso." ";
			$resulthorario=query($Sql,$link); 
			$datoshorario=mysql_fetch_array($resulthorario);
			$fechaentrada=$datoshorario[0];
                            ...
                            <td colspan="2">
                                <fieldset width='200' height='50'>
                                    ...
                                    <table align='center' border='0'>
                                        <tr>
                                            <td>Hora Ingreso</td>
                                            <td><input type='text' name='h_ingreso' onFocus="blur()" value="<?php echo $fechaentrada ?>" /></td>
                                        </tr>
                                        <tr>
                                            <td>Hora Salida</td>
                                            <td><input type='text' name='h_salida' onFocus="blur()" value="<?php echo $fecha_salida ?>" /></td>
                                        </tr>
                                    </table>
                                </fieldset>            	
                            </td>
